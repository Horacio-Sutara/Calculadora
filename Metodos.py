...
def newton_raphson(funcion,a,b,tol,iteraciones,res_1,funciona,metodo, tipo=False):
    ...
    if funciona:
        ...
        extremo_derecho=b
        extremo_izquierdo=a
        res_2,funciona=metodo(extremo_derecho,tol,funcion,iteraciones).calculo()if tipo==False else Metodo_Newton_Raphson(extremo_derecho,tol,funcion,iteraciones).calculo_raices_multiples()

        resta=valor=(abs(a)+abs(b))*0.01
        while funciona:
            if res_2 not in raices and res_2/(raices[-1]if len(raices)!=0 else res_2*2)<(1-tol*100) and a<=res_2<=b:
                raices.append(res_2)
                resta=valor
            else: 
                resta*=2
        ...
        while funciona:
            #print("divisiones")
            if res_1 not in raices and band==False and res_1/(raices[0]if len(raices)!=0 else res_1*2)<(1-tol*100) and a<=res_1<=b or res_1 not in raices and band and res_1/raices[-1]<(1-tol*100) and a<=res_1<=b :
                raices.append(res_1)
                band=True
                resta=valor
            else: resta*=2
            if res_1 in raices and res_1!=(raices[-1]if len(raices)!=0 else "")and res_1!=(raices[0]if len(raices)!=0 else "") or res_1==res_2 :
                return extremo_izquierdo,extremo_derecho,raices,funciona
            extremo_izquierdo=(extremo_izquierdo+resta)if (extremo_izquierdo+resta)!=0 else 0.1
            if extremo_derecho<extremo_izquierdo:
                return extremo_izquierdo,extremo_derecho,raices,funciona
            res_1,funciona=metodo(extremo_izquierdo,tol,funcion,iteraciones).calculo() if tipo==False else Metodo_Newton_Raphson(extremo_izquierdo,tol,funcion,iteraciones).calculo_raices_multiples()
